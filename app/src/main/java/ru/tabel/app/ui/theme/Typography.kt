...
// Используем системный шрифт (без внешних файлов шрифтов)
...
fun TextStyle.withColor(color: Color)           = copy(color = color)
fun TextStyle.withWeight(fontWeight: FontWeight) = copy(fontWeight = fontWeight)
fun TextStyle.withSize(fontSize: androidx.compose.ui.unit.TextUnit) = copy(fontSize = fontSize)
fun TextStyle.withAlign(textAlign: TextAlign)   = copy(textAlign = textAlign)
fun TextStyle.withAlpha(alpha: Float)           = copy(color = color.copy(alpha = alpha))
